...
namespace Rssdp.Infrastructure {
	...
	public sealed class SsdpCommunicationsServer : DisposableManagedObjectBase, ISsdpCommunicationsServer {
		...
		*/
		...
		/// </summary>
		public DeviceNetworkType DeviceNetworkType => _SocketFactory.DeviceNetworkType;
		...
		private IUdpSocket ListenForBroadcastsAsync() {
			IUdpSocket socket = _SocketFactory.CreateUdpMulticastSocket(_MulticastTtl,SsdpConstants.MulticastPort);
		...
		private async void ListenToSocket( IUdpSocket socket ) {
			...
			try {
				await TaskEx.Run(async () => {
					...
					while( !cancelled ) {
						try {
							ReceivedUdpData result = await socket.ReceiveAsync().ConfigureAwait(false);
							...
							if( result.ReceivedBytes > 0 ) {
								...
								Task processTask = TaskEx.Run(processWork);
							}
		...
		private async Task ReconnectBroadcastListeningSocket() {
			...
			while( !success ) {
				try {
					IUdpSocket oldSocket = _BroadcastListenSocket;
		...
		private void OnRequestReceived( HttpRequestMessage data,UdpEndPoint endPoint ) {
			...
			EventHandler<RequestReceivedEventArgs> handlers = RequestReceived;
			if( handlers != null ) {
		...
		private void OnResponseReceived( HttpResponseMessage data,UdpEndPoint endPoint ) {
			EventHandler<ResponseReceivedEventArgs> handlers = ResponseReceived;
			if( handlers != null ) {
